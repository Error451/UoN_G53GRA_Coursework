...
vec3 CalLight(Light light, vec3 fragNorm, vec3 fragPos, vec3 viewDir)
{
	//light direction
	vec3 lightDir;
	...
	vec3 diffuse = light.diffuse * diffuseTerm * material.diffuse;
	vec3 specular = light.specular * specularTerm * material.specular;
...
void main()
{
	...
	//for loop for lighting calculation - expensive but no choice in forward shading
	for(int i = 0; i < NR_LIGHT; i++)
	{
		if(lights[i].active != 0)
		{
			result += CalLight(lights[i], viewNorm, viewPos, viewerDir);
		}
	}
